...
// Debug logging to ~/Library/Application Support/Junofy/debug.log
...
static bool isLivePerformanceParam(int idx)
{
  return idx == kTuning || idx == kTranspose || idx == kHold ||
         ...
         idx == kArpQuantize || idx == kLfoQuantize || idx == kEarComfort ||
         (idx >= kFxDelayPingPong && idx <= kMasterLimiter) ||
      ...
      .withOutput("Output", juce::AudioChannelSet::stereo(), true))
{
  ...
  SFV fmtOutDb = [](float v, int) {
    float db = juce::jmap(v, 0.f, 1.f, -24.f, 12.f);
    return juce::String(db, 1) + " dB";
  };
  VFS parseOutDb = [](const juce::String& text) -> float {
    float db = juce::jlimit(-24.f, 12.f, text.getFloatValue());
    return juce::jmap(db, -24.f, 12.f, 0.f, 1.f);
  };
  ...
  addSlider(kEarComfort, "Ear comfort", 0.f, 0.f, 1.f, fmtPct, parsePct);

  addBool(kFxDelayPingPong, "Ping-pong delay", false);
  // Hall: 0 = dry; any wet >0 is at least 10% up to 100%. Defaults + room/damp/width = big plate.
  addSlider(kFxReverbMix, "Hall (plate)", 0.1f, 0.f, 1.f, fmtPct, parsePct);
  addSlider(kFxReverbRoom, "Plate size", 0.88f, 0.f, 1.f, fmtPct, parsePct);
  addSlider(kFxReverbDamping, "Plate damp", 0.42f, 0.f, 1.f, fmtPct, parsePct);
  addSlider(kFxReverbWidth, "Plate width", 0.96f, 0.f, 1.f, fmtPct, parsePct);
  addBool(kFxDelayBeforeReverb, "Delay before Hall", true);
  addSlider(kFxWarmth, "Output warmth", 0.f, 0.f, 1.f, fmtPct, parsePct);
  addSlider(kFxTapeWow, "Tape wow", 0.f, 0.f, 1.f, fmtPct, parsePct);
  addSlider(kMasterComp, "Master comp", 0.65f, 0.f, 1.f, fmtPct, parsePct);
  addSlider(kMasterOutGain, "Output gain", 0.5f, 0.f, 1.f, fmtOutDb, parseOutDb);
  addSlider(kMasterLimiter, "Master LIM", 1.f, 0.f, 1.f, fmtPct, parsePct);
  ...
  for (int j = 0; j < kNumFactoryPresets; j++)
  {
    ...
    if (kFxReverbMix < kNumParams)
      p.values[kFxReverbMix] = 0.1f;
    if (kFxReverbRoom < kNumParams)
      p.values[kFxReverbRoom] = 0.88f;
    if (kFxReverbDamping < kNumParams)
      p.values[kFxReverbDamping] = 0.42f;
    if (kFxReverbWidth < kNumParams)
      p.values[kFxReverbWidth] = 0.96f;
    if (kFxDelayBeforeReverb < kNumParams)
      p.values[kFxDelayBeforeReverb] = 1.f;
    if (kMasterComp < kNumParams)
      p.values[kMasterComp] = 0.65f;
    if (kMasterOutGain < kNumParams)
      p.values[kMasterOutGain] = 0.5f;
...
void KR106AudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
  ...
  mEarComfortSampleRate = sampleRate;
  mEarComfortLastCutoffHz = -1.f;
  {
    juce::dsp::ProcessSpec earSpec { sampleRate, (juce::uint32) samplesPerBlock, 1 };
    for (auto& f : mEarComfortFilters)
    {
      f.prepare (earSpec);
      f.reset();
    }
  ...
  mFxSampleRate = sampleRate;
  {
    juce::dsp::ProcessSpec dSpec { sampleRate, (juce::uint32) samplesPerBlock, 1 };
    mDelayL.prepare (dSpec);
    mDelayR.prepare (dSpec);
    mDelayL.reset();
    mDelayR.reset();
    mDelayFbLpL = 0.f;
    mDelayFbLpR = 0.f;
  ...
  {
    juce::dsp::ProcessSpec wSpec { sampleRate, (juce::uint32) samplesPerBlock, 1 };
    mWowL.prepare (wSpec);
    mWowR.prepare (wSpec);
    mWowL.reset();
    mWowR.reset();
  ...
  {
    juce::dsp::ProcessSpec rSpec { sampleRate, (juce::uint32) samplesPerBlock, 2 };
    mReverb.prepare (rSpec);
    mReverb.reset();
  }
  {
    const int limCh = juce::jmax (1, getTotalNumOutputChannels());
    juce::dsp::ProcessSpec limSpec { sampleRate, (juce::uint32) samplesPerBlock, (juce::uint32) limCh };
    mMasterCompressor.prepare (limSpec);
    mMasterCompressor.reset();
    mMasterLimiter.prepare (limSpec);
    mMasterLimiter.reset();
    mMasterLimiter.setThreshold (-0.5f);
    mMasterLimiter.setRelease (50.0f);
  }
                                        ...
                                        juce::MidiBuffer& midiMessages)
{
  ...
  applyEarComfort (buffer, nOutputs, nFrames);
  applyFxTapeWow (buffer, nOutputs, nFrames);
  if (getParamValue (kFxDelayBeforeReverb) > 0.5f)
  {
    applyFxDelay (buffer, nOutputs, nFrames);
    applyFxReverb (buffer, nOutputs, nFrames);
  }
  else
  {
    applyFxReverb (buffer, nOutputs, nFrames);
  ...
  applyFxWarmth (buffer, nOutputs, nFrames);
  applyMasterCompressor (buffer, nOutputs, nFrames);
  applyMasterOutputGain (buffer, nOutputs, nFrames);
  applyMasterLimiter (buffer, nOutputs, nFrames);

  // --- Peak detection (post master chain) for clip LED ---
  {
    float peak = 0.f;
...
void KR106AudioProcessor::parameterChanged(int paramIdx, float newValue)
{
  ...
  else if (paramIdx == kEarComfort)
  {
  }
  else if (paramIdx >= kFxDelayPingPong && paramIdx <= kMasterLimiter)
  {
  }
  else
  {
...
void KR106AudioProcessor::applyEarComfort (juce::AudioBuffer<float>& buffer,
                                           int nOutputs,
                                           int numSamples)
{
  const float amount = getParamValue (kEarComfort);
  if (amount <= 0.0001f)
    return;

  const double sr = mEarComfortSampleRate;
  const float nyquist = static_cast<float> (sr * 0.499);
  const float t = juce::jlimit (0.f, 1.f, amount);
  // Full wet ~8-10 kHz (plan): gentle treble cushion, not a dark band-limit.
  const float cutoffHz = juce::jmap (t, 0.f, 1.f, nyquist * 0.98f, 9000.f);

  if (std::abs (cutoffHz - mEarComfortLastCutoffHz) > 0.5f)
  {
    mEarComfortLastCutoffHz = cutoffHz;
    auto coeffs = juce::dsp::IIR::Coefficients<float>::makeLowPass (sr, static_cast<double> (cutoffHz), 0.707f);
    for (auto& f : mEarComfortFilters)
      *f.coefficients = *coeffs;
  }

  const int nCh = juce::jmin (nOutputs, buffer.getNumChannels());
  for (int c = 0; c < nCh; ++c)
  {
    float* ch = buffer.getWritePointer (c);
    for (int i = 0; i < numSamples; ++i)
      ch[i] = mEarComfortFilters[c].processSample (ch[i]);
    mEarComfortFilters[c].snapToZero();
  }
}

void KR106AudioProcessor::applyFxTapeWow (juce::AudioBuffer<float>& buffer, int nOutputs, int numSamples)
{
  const float amt = getParamValue (kFxTapeWow);
  if (amt <= 0.0001f)
    return;

  float* p0 = buffer.getWritePointer (0);
  float* p1 = buffer.getNumChannels() > 1 ? buffer.getWritePointer (1) : nullptr;
  const bool stereo = (p1 != nullptr && p1 != p0);

  const float sr = (float) mFxSampleRate;
  const float rateHz = juce::jmap (amt, 0.f, 1.f, 0.2f, 0.75f);
  const float inc = rateHz / sr;
  const float depthSamp = juce::jmap (amt, 0.f, 1.f, 0.6f, 4.f);
  const float base = 20.f;
  const float blend = juce::jmap (amt, 0.f, 1.f, 0.f, 0.26f);

  for (int i = 0; i < numSamples; ++i)
  {
    const float wL = std::sin (mWowPhase * juce::MathConstants<float>::twoPi);
    const float wR = std::sin ((mWowPhase + 0.217f) * juce::MathConstants<float>::twoPi);
    mWowPhase += inc;
    if (mWowPhase >= 1.f)
      mWowPhase -= 1.f;

    const float maxD = (float) mWowL.getMaximumDelayInSamples() - 2.f;
    float dL = juce::jlimit (2.f, maxD, base + depthSamp * wL);
    float dR = juce::jlimit (2.f, maxD, base + depthSamp * wR);

    float inL = p0[i];
    float delayedL = mWowL.popSample (0, dL);
    mWowL.pushSample (0, inL);
    p0[i] = inL * (1.f - blend) + delayedL * blend;

    if (stereo)
    {
      float inR = p1[i];
      float delayedR = mWowR.popSample (0, dR);
      mWowR.pushSample (0, inR);
      p1[i] = inR * (1.f - blend) + delayedR * blend;
    }
  }

  juce::ignoreUnused (nOutputs);
}

void KR106AudioProcessor::applyFxDelay (juce::AudioBuffer<float>& buffer, int nOutputs, int numSamples)
{
  const bool on = getParamValue (kFxDelayPingPong) > 0.5f;
  if (!on)
  {
    if (mFxDelayPingPongPrev)
    {
      mDelayL.reset();
      mDelayR.reset();
      mDelayFbLpL = mDelayFbLpR = 0.f;
    }
    mFxDelayPingPongPrev = false;
    return;
  }
  mFxDelayPingPongPrev = true;

  // Fixed ping-pong: cross-feedback between L/R delay lines; ~4 audible repeats, 10% wet.
  static constexpr float kPingPongDelayMs = 200.f;
  static constexpr float kPingPongMix     = 0.10f;
  static constexpr float kPingPongFb      = 0.58f;
  static constexpr float kPingPongToneHz  = 4800.f;

  float* p0 = buffer.getWritePointer (0);
  float* p1 = buffer.getNumChannels() > 1 ? buffer.getWritePointer (1) : nullptr;
  const bool stereo = (p1 != nullptr && p1 != p0);

  float delaySamps = static_cast<float> (kPingPongDelayMs * 0.001 * mFxSampleRate);
  delaySamps = juce::jlimit (2.f, (float) mDelayL.getMaximumDelayInSamples() - 2.f, delaySamps);

  const float fbLpCoeff =
      1.f - std::exp (-2.f * juce::MathConstants<float>::pi * kPingPongToneHz / (float) mFxSampleRate);

  for (int i = 0; i < numSamples; ++i)
  {
    float inL = p0[i];
    float inR = stereo ? p1[i] : inL;

    float delayedL = mDelayL.popSample (0, delaySamps);
    float delayedR = mDelayR.popSample (0, delaySamps);

    mDelayFbLpL += fbLpCoeff * (delayedL - mDelayFbLpL);
    mDelayFbLpR += fbLpCoeff * (delayedR - mDelayFbLpR);

    mDelayL.pushSample (0, inL + kPingPongFb * mDelayFbLpR);
    mDelayR.pushSample (0, inR + kPingPongFb * mDelayFbLpL);

    if (stereo)
    {
      p0[i] = inL * (1.f - kPingPongMix) + delayedL * kPingPongMix;
      p1[i] = inR * (1.f - kPingPongMix) + delayedR * kPingPongMix;
    }
    else
    {
      const float wetMono = 0.5f * (delayedL + delayedR);
      p0[i] = inL * (1.f - kPingPongMix) + wetMono * kPingPongMix;
    }
  }
...
void KR106AudioProcessor::applyFxReverb (juce::AudioBuffer<float>& buffer, int nOutputs, int numSamples)
{
  const float raw = getParamValue (kFxReverbMix);
  if (raw <= 0.0001f)
    return;

  const float mix = juce::jlimit (0.1f, 1.f, raw);

  juce::Reverb::Parameters p;
  p.roomSize  = getParamValue (kFxReverbRoom);
  p.damping   = getParamValue (kFxReverbDamping);
  p.wetLevel  = mix;
  p.dryLevel  = 1.f - mix;
  p.width     = getParamValue (kFxReverbWidth);
  p.freezeMode = 0.f;
  ...
  const bool stereo = nOutputs > 1 && buffer.getNumChannels() > 1;

  if (stereo)
  {
    auto stereoBlock = sub.getSubsetChannelBlock (0, 2);
    juce::dsp::ProcessContextReplacing<float> ctx (stereoBlock);
    mReverb.process (ctx);
  }
  else
  {
    auto monoBlock = sub.getSubsetChannelBlock (0, 1);
...
void KR106AudioProcessor::applyFxWarmth (juce::AudioBuffer<float>& buffer, int nOutputs, int numSamples)
{
  const float w = getParamValue (kFxWarmth);
  if (w <= 0.0001f)
    return;

  const float drive = 1.f + 3.5f * w;
  const int nCh = juce::jmin (nOutputs, buffer.getNumChannels());
  for (int c = 0; c < nCh; ++c)
  {
    float* ch = buffer.getWritePointer (c);
...
void KR106AudioProcessor::applyMasterCompressor (juce::AudioBuffer<float>& buffer, int nOutputs, int numSamples)
{
  const float amt = getParamValue (kMasterComp);
  if (amt <= 0.0001f)
    return;

  const float threshDb = juce::jmap (amt, 0.f, 1.f, -34.f, -11.f);
  const float ratio = juce::jmap (amt, 0.f, 1.f, 1.8f, 6.0f);
  mMasterCompressor.setThreshold (threshDb);
  mMasterCompressor.setRatio (ratio);
  mMasterCompressor.setAttack (3.f);
  mMasterCompressor.setRelease (150.f);
  ...
  // Makeup: quiet patches get lifted toward loud; hot peaks were reduced by the comp.
  const float makeupDb = juce::jmap (amt, 0.f, 1.f, 1.5f, 16.f);
  buffer.applyGain (juce::Decibels::decibelsToGain (makeupDb));
}

void KR106AudioProcessor::applyMasterOutputGain (juce::AudioBuffer<float>& buffer, int nOutputs, int numSamples)
{
  juce::ignoreUnused (nOutputs, numSamples);
  const float db = juce::jmap (getParamValue (kMasterOutGain), 0.f, 1.f, -24.f, 12.f);
  buffer.applyGain (juce::Decibels::decibelsToGain (db));
}

void KR106AudioProcessor::applyMasterLimiter (juce::AudioBuffer<float>& buffer, int nOutputs, int numSamples)
{
  const float amt = getParamValue (kMasterLimiter);
  if (amt <= 0.0001f)
    return;

  // Stronger limiter amount → threshold closer to 0 dBFS (ceiling).
  mMasterLimiter.setThreshold (juce::jmap (amt, 0.f, 1.f, -4.0f, -0.2f));
  mMasterLimiter.setRelease (juce::jmap (amt, 0.f, 1.f, 90.f, 35.f));
