...
public:
  ...
  // Short linear ramp after a non-legato trigger (phase reset + fast envelope edge).
  // Reduces note-on clicks; skipped when isRetrigger (legato / voice steal).
  int mOutputAttackSamplesRemaining = 0;
  int mOutputAttackTotalSamples     = 0;
  ...
  void Trigger(double level, bool isRetrigger)
  {
    ...
    {
      mOsc.Reset();
      mOutputAttackSamplesRemaining = mOutputAttackTotalSamples;
    ...
    {
      mOutputAttackSamplesRemaining = 0;
    }
  }
  ...
  void SetSampleRateAndBlockSize(double sampleRate, int blockSize)
  {
    ...
    // ~1.2 ms output ramp on fresh triggers (sample-rate scaled, min ~8 samples)
    mOutputAttackTotalSamples = std::max(
        8, static_cast<int>(std::lround(static_cast<double>(mSampleRate) * 0.0012)));
                                  ...
                                  int nFrames)
  {
    ...
    for (int i = startIdx; i < startIdx + nFrames; i++)
    {
      ...
      if (mOutputAttackSamplesRemaining > 0)
      {
        const int total = mOutputAttackTotalSamples;
        const int done  = total - mOutputAttackSamplesRemaining;
        vcaOut *= static_cast<float>(done + 1) / static_cast<float>(total);
        --mOutputAttackSamplesRemaining;
      }
