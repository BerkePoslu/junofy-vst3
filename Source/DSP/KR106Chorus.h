...
struct Chorus
{
  ...
  // When chorus is bypassed, hardware is mono — we add gentle L/R decorrelation so
  // headphones/speakers aren't dead-centre (still mono-compatible when summed).
  float mDryStereoLp = 0.f;
  float mDryStereoAlpha = 0.02f;
  ...
  void Init(float sampleRate)
  {
    ...
    mDryStereoLp = 0.f;
    {
      const float tau = 0.0018f; // ~1.8 ms low-pass for side extraction
      mDryStereoAlpha = 1.f - std::exp(-1.f / (tau * sampleRate));
    }
  ...
  void Process(float input, float& outL, float& outR)
  {
    ...
    if (mFade <= 0.f)
    {
      auto fillDryStereo = [this](float in, float& oL, float& oR)
      {
        float lp = mDryStereoLp;
        lp += mDryStereoAlpha * (in - lp);
        mDryStereoLp = lp;
        const float side = in - lp;
        static constexpr float kSpread = 0.30f;
        oL = in + kSpread * side;
        oR = in - kSpread * side;
      };

      if (mLine0.mBuf.empty())
      {
        fillDryStereo(input, outL, outR);
        return;
      }
